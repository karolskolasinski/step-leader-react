...
const Dashboard = () => {
  ...
  const { monthlySteps, status, lastUpdate, error } = useSelector((state: RootState) =>
    state.steps
  );
    ...
    <div className="bg-white shadow rounded-lg p-6 max-w-2xl mx-auto">
      ...
      <div className="bg-gray-50 p-6 rounded-lg">
        ...
        <p className="text-sm text-gray-600">
          Cel: 10 000 kroków dziennie ({(10000 * new Date().getDate()).toLocaleString()}{" "}
          kroków w tym miesiącu)
        </p>
      </div>
    </div>
  );
